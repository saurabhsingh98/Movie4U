...
const MovieCard = ({ movie }) => {
    // Use a local variable for the poster
    const posterUrl = movie?.Poster === "N/A" ? DEFAULT_POSTER : movie?.Poster
    ...
    return (
        <div className='bg-gray-800 rounded-lg overflow-hidden shadow-lg transform transition duration-300 hover:scale-105 hover:shadow-2xl' onClick={handleMovieDetails}>
            <div className='h-48 bg-gradient-to-r from-purple-500 to-pink-500' style={{ backgroundImage: `url(${posterUrl})` }}></div>
            <div className='p-6'>
